...
TQS는 **Keural 사용자의 토큰 사용량을 제한**하는 독립형 FastAPI 마이크로서비스입니다.

- 사용자당 **50,000 토큰** 소진 시 → **5시간 차단**
- 5시간 후 **자동 해제** (MongoDB 기준)
- Spring API 관리자가 **즉시 수동 해제** 가능 (`POST /v1/admin/reset`)
- 데이터베이스: **MongoDB** (컬렉션: `tqs_users`)
- TQS에 장애가 발생해도 **fail-open** — 사용자 요청은 통과됨 (경고 로그만 기록)
...
**핵심 포인트:**
- Next.js와 Spring은 TQS를 **직접 호출하지 않습니다** (Spring의 admin reset 제외)
- TQS를 호출하는 것은 **Keural FastAPI 하나뿐**입니다
...
> `TQS_ADMIN_API_KEY` 값을 사용합니다. `TQS_API_KEY`와 **다른 키**입니다.
...
**레포지토리:** `mkd-keural-be-fastapi` → `feat/tqs-integration` 브랜치 (Enrique 검토 대기 중)

**파일 1:** `app/service/tqs/client.py`
- `tqs_check()` — 비동기 pre-flight 확인
- `tqs_consume()` — 비동기 post-flight 기록 (fire-and-forget)

**파일 2:** `app/service/chat/process.py`
- `[2.5]` 구간: `stream_agent()` 호출 직전에 `tqs_check()` 실행
- `finally` 블록: 응답 완료 후 `tqs_consume()` 실행

**파일 3:** `app/config/constants.py`
- `TQS_ENABLED: bool = False` — 기본값 비활성화 (배포 시 `True`로 변경)
- `TQS_BASE_URL: str = "http://tqs-service:8001"`
- `TQS_API_KEY: str = ""`
...
├── docker/
│   └── docker-compose.yml       # MongoDB + TQS 컨테이너
├── Dockerfile
├── pyproject.toml
├── .env.dev                     # 도커 환경 설정
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `TQS_MONGO_URL` | `mongodb://localhost:27017` | MongoDB 연결 주소 |
| `TQS_MONGO_DB` | `tls` | 데이터베이스 이름 |
| `TQS_API_KEY` | `sk-tqs-changeme` | Keural FastAPI에서 사용하는 서비스 키 |
| `TQS_ADMIN_API_KEY` | `sk-tqs-admin-changeme` | Spring API 관리자 키 (별도) |
| `TQS_TOKEN_LIMIT` | `50000` | 사용자당 토큰 한도 |
| `TQS_RESET_HOURS` | `5` | 차단 후 자동 해제까지 시간 |
...
```bash
# 레포지토리 클론
git clone git@github-mkd:MKD-CORP/mkd-keural-api-tqs.git
cd mkd-keural-api-tqs

# 도커로 실행 (MongoDB + TQS 함께 시작)
...
1. **관리자 수동 해제** — Spring에서 `POST /v1/admin/reset`을 호출할 때 사용할 `TQS_ADMIN_API_KEY` 값을 어떻게 공유하면 좋을까요?

2. **user_id 형식** — 현재 Keural FastAPI에서 전달하는 user_id는 문자열(예: `"user_12345"`) 형태입니다. Spring에서도 동일한 형식을 사용하고 있는지 확인 부탁드립니다.

3. **TQS 서버 네트워크 설정** — TQS는 현재 `keural-network` Docker 네트워크에서 `keural-tqs`라는 이름으로 실행됩니다 (`pipeline.dev.sh` 기준). Spring에서 이 주소로 접근 가능한지 확인이 필요합니다.

4. **기타 수정 제안** — 구조나 API 설계에서 개선이 필요한 부분이 있으면 말씀해 주세요.

---

**레포지토리:** `git@github-mkd:MKD-CORP/mkd-keural-api-tqs.git`
